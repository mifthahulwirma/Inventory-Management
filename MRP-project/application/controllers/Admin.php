...
class Admin extends CI_Controller
{
	...
	public function index()
	{
		$data = [
			...
			// 'on_progress' => $this->CrudModel->getData('plan_shift', array('ps_status' => 1))->num_rows(),

			'content' => 'admin/beranda',
			'navlink' => 'beranda',

			// Staff Status
			'readyCount' => $this->db->query("SELECT COUNT(*) AS count FROM staff WHERE st_status = 0")->row()->count,
			'scheduledCount' => $this->db->query("SELECT COUNT(*) AS count FROM staff WHERE st_status = 1")->row()->count,

			// Machine Status
			'workingCount' => $this->db->query("SELECT COUNT(*) AS count FROM machine WHERE mc_status = 2")->row()->count,
			'idleCount' => $this->db->query("SELECT COUNT(*) AS count FROM machine WHERE mc_status = 1")->row()->count,

			// Format data for Chart.js
			'labels' => ['Ready', 'Scheduled'],
			'machineLabels' => ['Ready', 'Used']
		];
	...
	public function detail_production()
	{
		if ($this->uri->segment(4) === 'view') {
			...
			$tampil = $this->db->query(
                ...
                AND project.id_product = product.id_product'
			)->row();
	...
	public function machine()
	{
		if ($this->uri->segment(3) === 'addmachine') {
			...
			$data = [
				'planshift' => $this->db->query('SELECT * FROM plan_shift JOIN staff WHERE plan_shift.id_staff=staff.id_staff AND ps_status = 1 AND staff.st_status=1')->result(),
